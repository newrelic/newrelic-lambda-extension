...
resource "aws_iam_role" "newrelic_terraform_example_nodejs_role" {
  name               = "newrelic_terraform_example_nodejs_role"
  assume_role_policy = file("./lambda-assume-role-policy.json")
}

resource "aws_iam_role_policy" "newrelic_terraform_example_nodejs_role_policy" {
  name   = "newrelic_terraform_example_nodejs_role_policy"
  role   = aws_iam_role.newrelic_terraform_example_nodejs_role.id
  policy = file("./lambda-policy.json")
}

resource "aws_iam_role_policy_attachment" "newrelic_license_key_policy_attachment" {
  role       = aws_iam_role.newrelic_terraform_example_nodejs_role.name
  policy_arn = data.aws_iam_policy.newrelic_license_key_policy.arn
}

resource "aws_lambda_function" "newrelic_terraform_example_nodejs_function" {
  description = "A simple Lambda function, with New Relic telemetry"
  depends_on = [
    aws_cloudwatch_log_group.newrelic_terraform_example_nodejs_log_group,
    aws_iam_role.newrelic_terraform_example_nodejs_role,
  ...
  role    = aws_iam_role.newrelic_terraform_example_nodejs_role.arn
  ...
  layers = [var.newrelic_nodejs_layer]
}

resource "aws_cloudwatch_log_group" "newrelic_terraform_example_nodejs_log_group" {
